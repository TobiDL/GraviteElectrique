...
public class EditeurDeNiveaux extends JPanel {
	...
	private double largeurCible = 75;
	...
	/**
	 * Methode utilsee pour dessiner la barre latterale gauche ainsi que les éléments qui seront présents sur l'interface
	 */
	@Override
	public void paintComponent(Graphics g){
		...
		obstaclePhysiqueCarre = new ObstaclePhysique(100, 100/4, ((150/2)-(100/2)), 100);
		...
		canon = new Canon(0,posXCanon + deplacementCanonX,440,100);
		...
		addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent arg0) {
				...
				if(surLaCible){
					...
					System.out.println("x: "+(deplacementCibleX+ posXCible) + " Y: "+ (posYCible + deplacementCibleY));
				}
				if(surUnOE){
					if (surLeModelOE){
						surLeModelOE = false;
					...
					if(!obstaclesElectriques.isEmpty()){
						...
						System.out.println("x: "+(posXOE + deplacementOEX) + " Y: "+ (posYOE + deplacementOEY));
				...
				if(surUnOPR){
					...
					if(!obstaclesPhysiquesRond.isEmpty()){
						...
						System.out.println("x: "+(posXOPR + deplacementOPRX) + " Y: "+ (posYOPR + deplacementOPRY));
				...
				if(surUnOPC){
					if(surLeModelOPC){
						surLeModelOPC = false;
						ObstaclePhysique opc = new ObstaclePhysique(100, 100/4, arg0.getX(), arg0.getY());
					...
					if(!obstaclesPhysiquesCarre.isEmpty()){
						...
						System.out.println("x: "+(posXOPC + deplacementOPCX) + " Y: "+ (posYOPC + deplacementOPCY));
			...
			public void mousePressed(MouseEvent e) {
				...
				if(cible.surLaCible(e.getX(), e.getY(),matTransfo)){
					if(e.getButton()==1){
						...
						String cible=JOptionPane.showInputDialog(null,"Longueur de la cible","Cible",JOptionPane.OK_CANCEL_OPTION);
						if (cible != null && !cible.trim().equals("") &&  cible.matches("([0-9])+") && Long.parseLong(cible)>0&& Long.parseLong(cible)<301) {
							largeurCible =Integer.parseInt(cible);
							System.out.println(largeurCible);
						} else {
							if(cible!=null && cible.trim().equals("")  ){
								JOptionPane.showMessageDialog(null, "Vous devez entrer une valeur.", "Erreur", JOptionPane.ERROR_MESSAGE);
							} else {
								JOptionPane.showMessageDialog(null, "Ta longueur doit etre comprise entre 50 et 300 \net doit etre enti\u00E8re.", "Erreur", JOptionPane.ERROR_MESSAGE);
							}
						}	
					}
				}
					...
					for(int k = 0 ; k < obstaclesElectriques.size(); k++){
						if(obstaclesElectriques.get(k).contains(e.getX(), e.getY(),matTransfo)){
							...
							if(e.getButton()==1){
								...
								if (charge != null && !charge.trim().equals("") &&  charge.matches("(-)?([0-9])+") && Integer.parseInt(charge)<11&& Integer.parseInt(charge)>-11 && Integer.parseInt(charge)!=0 ) {
									obstaclesElectriques.get(obsChoisit).setCharge(Integer.parseInt(charge));
								} else {
									if(charge!=null && charge.trim().equals("")  ){
										JOptionPane.showMessageDialog(null, "Vous devez entrer une valeur.", "Erreur", JOptionPane.ERROR_MESSAGE);
									} else {
										JOptionPane.showMessageDialog(null, "Ta charge doit etre comprise entre 10 et -10 \net doit etre enti\u00E8re et diff\u00E9rente de 0", "Erreur", JOptionPane.ERROR_MESSAGE);
									}
								}									
								surUnOE = false;
							}
					...
					for(int k = 0 ; k < obstaclesPhysiquesRond.size(); k++){
						if(obstaclesPhysiquesRond.get(k).surLObstacle(e.getX(), e.getY(),matTransfo)){
							...
							if(e.getButton()==3){
								...
								if (rayon != null && !rayon.trim().equals("") && rayon.matches("([0-9])+") && Long.parseLong(rayon)<126&& Long.parseLong(rayon)>24 ) {
									obstaclesPhysiquesRond.get(obsPhys).setRayon(Long.parseLong(rayon));
								} else {
									if(rayon!=null && rayon.trim().equals("")){
										JOptionPane.showMessageDialog(null, "Vous devez entrer une valeur.", "Erreur", JOptionPane.ERROR_MESSAGE);
									} else {
										JOptionPane.showMessageDialog(null, "Ton rayon doit etre comprise entre 25 et 125 \net doit etre enti\u00E8re.", "Erreur", JOptionPane.ERROR_MESSAGE);
									}
								}													
								surUnOPR = false;
							}
						...
						if(obstaclesPhysiquesCarre.get(k).surLObstacle(e.getX(), e.getY(),matTransfo)){
							...
							if(e.getButton()==3){
								...
								if(hauteur.getText()!=null && largeur.getText()!=null && !hauteur.getText().trim().equals("") && !largeur.getText().trim().equals("")){
									...
									if(longueurs<351&&longueurs>124 && largeur.getText().matches("([0-9])+")){
										obstaclesPhysiquesCarre.get(obsPhys2).setLongueur(longueurs);
									}else{
										JOptionPane.showMessageDialog(null, "La longueur doit etre comprise entre 125 et 350 \net doit etre enti\u00E8re." ,"Erreur", JOptionPane.ERROR_MESSAGE);
									}	
									if(hauteurs<351 && hauteurs>124 && hauteur.getText().matches("([0-9])+")){
										obstaclesPhysiquesCarre.get(obsPhys2).setHauteur(hauteurs);
									} else {
										JOptionPane.showMessageDialog(null, "La hauteur doit etre comprise entre 125 et 350 \net doit etre enti\u00E8re." ,"Erreur", JOptionPane.ERROR_MESSAGE);
									}
								}
								else{
									JOptionPane.showMessageDialog(null, "Il manque une ou plusieurs des valeurs.", "Erreur", JOptionPane.ERROR_MESSAGE);
								}
								surUnOPC = false;
							}
						}
	...
	/**
	 * Methode permettant de retablir ou de reinitinaliser les parametres de lediteur de niveaux
	 */
	private void reset(){
		deplacementCanonX=0;
		deplacementCanonY=0;
	...
	public Niveaux getNiveau(){
		...
		niveau.getButtonRecommencer().setVisible(true);
		niveau.setNumero(6);
	...
	/**
	 * Methode utilisee pour effacer le niveau qui avait ete creer
	 */
	...
	/**
	 * Methode privee pour creer une fenetre avec deux champ pour la modification de la grandeur d'un obstacle physique carre
	 * @param frame
	 */
