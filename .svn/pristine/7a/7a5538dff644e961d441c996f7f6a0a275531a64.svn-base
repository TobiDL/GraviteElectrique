...
public class Projectile {
	...
	public void creer(AffineTransform matTransfo){			
		projectile = new Ellipse2D.Double(posX, posY, rayon+3, rayon+3);
	...
	public void dessiner(Graphics2D g2d,AffineTransform matTransfo){
		...
		if(charge >0){
			g2d.drawImage(sProjectileRouge, (int)sProj.getBounds2D().getX(),(int)sProj.getBounds2D().getY(), (int)sProj.getBounds2D().getWidth(), (int)sProj.getBounds2D().getHeight(),null);
			g2d.setColor(new Color(255,0,0,255));}

		if(charge<0){
			g2d.drawImage(sProjectileBleu, (int)sProj.getBounds2D().getX(),(int)sProj.getBounds2D().getY(), (int)sProj.getBounds2D().getWidth(), (int)sProj.getBounds2D().getHeight(),null);
			g2d.setColor(new Color(0,128,128,255));}

		if(charge==0){
			g2d.drawImage(sProjectileOrange, (int)sProj.getBounds2D().getX(),(int)sProj.getBounds2D().getY(), (int)sProj.getBounds2D().getWidth(), (int)sProj.getBounds2D().getHeight(),null);
	...
	/**
	 * Methode servant a modifier charge du projectile
	 * @param charge
	 */
	...
	/**
	 * methode qui permet de  déplacer en fonction du temps entré
	 */
	...
	public void collisonPhysique(ObstaclePhysique obsPhy){
		...
		vitesseYF = ((1-(masseObs/masse)) / ( 1+(masseObs/masse) )) * vitesseY*0.80;	
	...
	public void collisionElectrique( ObstacleElectrique obsE){		
		...
		if(!enCollisionElec){
			...
			// fin if
			System.out.println("modifAX "+modifAX);
			System.out.println("modifAY "+ modifAY);
		}
	}
	/**
	...
	/**
	 * Methode retablissant les parametres du projectile	 
	 */
	...
	/**
	 * Methode retournant si le projectile est en collision 
	 */
	...
	/**
	 * Methode retournant l'orientation du vecteur en fonction de la vitesse en X et en Y
	 */
	...
	/**
	 * Methode retournant la masse du projectile
	 */
	public double getMasse() {
		return masse;
	}
	/**
	 * Methode permetttant de modifier la masse du projectile
	 */
...
/**
*Methode prive utilise pour les images
*/
